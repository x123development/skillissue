...
public class CombatMenu implements InventoryMenu {
    ...
    public Inventory build(Player player) {
        ...
        miningOverviewMeta.setLore(List.of(ChatColor.GRAY+"current Exp: "+(sh.getSkillExpFor(uuid,Skills.COMBAT)-getExpByLvl(sh.getSkillLvlFor(uuid,Skills.COMBAT)))+"/"+(getExpByLvl(sh.getSkillLvlFor(uuid,Skills.COMBAT)+1)-getExpByLvl(sh.getSkillLvlFor(uuid,Skills.COMBAT))),
        ...
        if(sh.getSkillPerkFor(uuid,Skills.COMBAT)==1)
        ...
        if(sh.getSkillPerkFor(uuid,Skills.COMBAT)==2)
        ...
        if(sh.getSkillPerkFor(uuid,Skills.COMBAT)==3)
    ...
    public void handleEvent(InventoryClickEvent event) {
        ...
        switch(event.getRawSlot()){
            case 29:
                ...
                if(MainClass.INSTANCE.getSkillHandler().getSkillLvlFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT)>=10){
                    MainClass.INSTANCE.getSkillHandler().setSkillPerkFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT,1);
                    event.getWhoClicked().sendMessage(ChatColor.GREEN+"COMBAT perk 1 selected!");
                }else{
            ...
            case 31:
                event.getWhoClicked().closeInventory();
                if(MainClass.INSTANCE.getSkillHandler().getSkillLvlFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT)>=10){
                    MainClass.INSTANCE.getSkillHandler().setSkillPerkFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT,2);
                    event.getWhoClicked().sendMessage(ChatColor.GREEN+"COMBAT perk 2 selected!");
                }else{
                    event.getWhoClicked().sendMessage(ChatColor.DARK_RED+"Reach Level 10 to unlock perks for this skill!");
                }
                break;
            case 33:
                event.getWhoClicked().closeInventory();
                if(MainClass.INSTANCE.getSkillHandler().getSkillLvlFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT)>=10){
                    MainClass.INSTANCE.getSkillHandler().setSkillPerkFor(event.getWhoClicked().getUniqueId().toString(),Skills.COMBAT,3);
                    event.getWhoClicked().sendMessage(ChatColor.GREEN+"COMBAT perk 3 selected!");
                }else{
                    event.getWhoClicked().sendMessage(ChatColor.DARK_RED+"Reach Level 10 to unlock perks for this skill!");
                }
                break;
